      ...
      <li v-if="!pages.includes(1)">
        ...
        >
          <button
            ...
            @click="selectPageClickHandler(1)"
          >
            1
          </button>
        </div>
      ...
      <li v-for="page in pages" :key="`page-${page}`">
        ...
        >
          <button
            ...
            @click="selectPageClickHandler(page)"
          >
            {{ page }}
          </button>
        </div>
      ...
      <li v-if="!pages.includes(totalPages)">
        ...
        >
          <button
            ...
            @click="selectPageClickHandler(totalPages)"
          >
            {{ totalPages }}
          </button>
        </div>
...
<script lang="ts" setup>
  ...
  const emit = defineEmits<{
    (n: 'select-page', v: number): void
    (n: 'next-page', v: number): void
    (n: 'previous-page', v: number): void
  }>()
  ...
  const { totalPages, pages, selectedPage, startPage, endPage, next, prev, setPage } =
    usePagination({
      ...
      maxPages: Math.ceil(props.totalItems / props.pageSize),
    })

  const setPageQuery = async () => {
    await router.push({ query: { page: selectedPage.value } })
  }

  const selectPageClickHandler = async (page: number) => {
    setPage(page)
    await setPageQuery()

    emit('select-page', selectedPage.value)
  }
  ...
  const previousPageClickHandler = async () => {
    prev()
    await setPageQuery()
